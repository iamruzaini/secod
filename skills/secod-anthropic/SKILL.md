...
description: >-
  Help coding agents securely implement Anthropic features using version-matched official APIs, secure defaults, and provider-specific tests. Use when Anthropic Messages API, Claude SDKs, streaming, files, prompt caching, or tool use.
license: Apache-2.0
metadata:
...
# Secure Anthropic implementation

## Purpose

Implement Anthropic features securely. Provider-family router supplies shared context; this skill owns exact product decisions and version-qualified SDK/runtime use.

## When to use

Use when Anthropic Messages API, Claude SDKs, streaming, files, prompt caching, or tool use. Do not activate from package presence alone when current feature does not use Anthropic.
...
- Keep keys server-side, minimize data, validate tool inputs/output, authorize effects, and bound tokens/retries.
