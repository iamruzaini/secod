...
description: >-
  Help coding agents securely implement AWS Cognito features using version-matched official APIs, secure defaults, and provider-specific tests. Use when Cognito user pools, identity pools, app clients, hosted UI, JWTs, triggers, or federation.
license: Apache-2.0
metadata:
...
# Secure AWS Cognito implementation

## Purpose

Implement AWS Cognito features securely. Provider-family router supplies shared context; this skill owns exact product decisions and version-qualified SDK/runtime use.

## When to use

Use when Cognito user pools, identity pools, app clients, hosted UI, JWTs, triggers, or federation. Do not activate from package presence alone when current feature does not use AWS Cognito.
...
- Verify Cognito tokens and app-client purpose, constrain federation, and keep application authorization independent.
