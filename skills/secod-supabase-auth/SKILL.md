...
description: >-
  Help coding agents securely implement Supabase Auth features using version-matched official APIs, secure defaults, and provider-specific tests. Use when Supabase Auth sessions, JWTs, MFA, OAuth, email links, hooks, or SSR clients.
license: Apache-2.0
metadata:
...
Implement Supabase Auth features securely. Provider-family router supplies shared context; this skill owns exact product decisions and version-qualified SDK/runtime use.

## When to use

Use when Supabase Auth sessions, JWTs, MFA, OAuth, email links, hooks, or SSR clients. Do not activate from package presence alone when current feature does not use Supabase Auth.
