...
name: secod-neon
description: >-
  Help coding agents securely implement Neon features using version-matched official APIs, secure defaults, and provider-specific tests. Use when Neon Postgres, branches, pooled/direct connections, serverless driver, roles, or connection strings.
license: Apache-2.0
metadata:
...
# Secure Neon implementation

## Purpose

Implement Neon features securely. Provider-family router supplies shared context; this skill owns exact product decisions and version-qualified SDK/runtime use.

## When to use

Use when Neon Postgres, branches, pooled/direct connections, serverless driver, roles, or connection strings. Do not activate from package presence alone when current feature does not use Neon.
...
- Use scoped roles, TLS, parameterized queries, safe branch/environment separation, and bounded pooling.
