...
name: secod-whop
description: >-
  Help coding agents securely implement Whop features using version-matched official APIs, secure defaults, and provider-specific tests. Use when Whop API keys, checkout, memberships, access passes, apps, or webhooks.
license: Apache-2.0
metadata:
...
# Secure Whop implementation

## Purpose

Implement Whop features securely. Provider-family router supplies shared context; this skill owns exact product decisions and version-qualified SDK/runtime use.

## When to use

Use when Whop API keys, checkout, memberships, access passes, apps, or webhooks. Do not activate from package presence alone when current feature does not use Whop.
