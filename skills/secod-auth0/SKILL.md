...
name: secod-auth0
description: >-
  Help coding agents securely implement Auth0 features using version-matched official APIs, secure defaults, and provider-specific tests. Use when Auth0 SDKs, Auth0 tenant callbacks, Actions, access/refresh tokens, or Auth0 webhooks.
license: Apache-2.0
metadata:
...
Implement Auth0 features securely. Provider-family router supplies shared context; this skill owns exact product decisions and version-qualified SDK/runtime use.

## When to use

Use when Auth0 SDKs, Auth0 tenant callbacks, Actions, access/refresh tokens, or Auth0 webhooks. Do not activate from package presence alone when current feature does not use Auth0.
...
- Validate access tokens against fixed issuer/audience and map Auth0 subject to local identity.
