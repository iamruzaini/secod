...
description: >-
  Help coding agents preserve authorization, data correctness, and bounded resource use when databases, networks, queues, and providers fail.
license: Apache-2.0
metadata:
...
# Failure-safe application implementation

## Purpose

Own portable security invariants for failure-safe application implementation. Provider and framework adapters own exact SDK calls and deployment controls.

## When to use

Use for external calls, transactions, retries, queues, caches, webhooks, multi-step mutations, fallbacks, timeouts, or error handling. Do not activate for unrelated documentation or presentation-only changes.
