...
name: secod-convex
description: >-
  Help coding agents securely implement Convex features using version-matched official APIs, secure defaults, and provider-specific tests. Use when Convex queries, mutations, actions, HTTP actions, auth context, storage, or scheduled functions.
license: Apache-2.0
metadata:
...
# Secure Convex implementation

## Purpose

Implement Convex features securely. Provider-family router supplies shared context; this skill owns exact product decisions and version-qualified SDK/runtime use.

## When to use

Use when Convex queries, mutations, actions, HTTP actions, auth context, storage, or scheduled functions. Do not activate from package presence alone when current feature does not use Convex.
...
- Authorize every public function with trusted identity, validate arguments, and prevent cross-tenant reads/writes.
