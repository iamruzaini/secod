...
description: >-
  Help coding agents securely implement Dodo Payments features using version-matched official APIs, secure defaults, and provider-specific tests. Use when Dodo Payments checkout, subscriptions, licenses, API clients, mobile checkout, or webhooks.
license: Apache-2.0
metadata:
...
# Secure Dodo Payments implementation

## Purpose

Implement Dodo Payments features securely. Provider-family router supplies shared context; this skill owns exact product decisions and version-qualified SDK/runtime use.

## When to use

Use when Dodo Payments checkout, subscriptions, licenses, API clients, mobile checkout, or webhooks. Do not activate from package presence alone when current feature does not use Dodo Payments.
...
- Keep API keys server-side, trust catalog/customer state, verify webhooks, deduplicate events, and reconcile entitlements.
...
- [Secure implementation recipe](references/checkout-webhooks.md) — provider-specific boundaries and coding sequence.
