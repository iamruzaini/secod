...
name: secod-workos
description: >-
  Help coding agents securely implement WorkOS features using version-matched official APIs, secure defaults, and provider-specific tests. Use when WorkOS AuthKit, SSO, Directory Sync, organizations, sessions, or events.
license: Apache-2.0
metadata:
...
# Secure WorkOS implementation

## Purpose

Implement WorkOS features securely. Provider-family router supplies shared context; this skill owns exact product decisions and version-qualified SDK/runtime use.

## When to use

Use when WorkOS AuthKit, SSO, Directory Sync, organizations, sessions, or events. Do not activate from package presence alone when current feature does not use WorkOS.
...
- Bind WorkOS organization and immutable subject to local tenant membership and reconcile lifecycle events.
