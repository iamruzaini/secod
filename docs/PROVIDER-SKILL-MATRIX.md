...
This table groups the complete `secod/skills` catalog by provider. All skills for the same provider appear in one row. `Generalized` contains provider-neutral security skills that apply across technologies.
...
| Better Auth | Secure Better Auth configuration, server handlers, sessions, plugins, and identity flows. | `secod-better-auth` | 1 |
| Clerk | Secure Clerk authentication, middleware, sessions, authorization, environment configuration, and webhooks. | `secod-clerk` | 1 |
| Cloudflare | Secure the Cloudflare account and developer platform, including Workers, Pages, Queues, Workflows, Hyperdrive, Vectorize, Workers AI, and AI Gateway. | `secod-cloudflare`<br>`secod-cloudflare-workers`<br>`secod-cloudflare-pages`<br>`secod-cloudflare-queues`<br>`secod-cloudflare-workflows`<br>`secod-cloudflare-hyperdrive`<br>`secod-cloudflare-vectorize`<br>`secod-cloudflare-workers-ai`<br>`secod-cloudflare-ai-gateway` | 9 |
| Convex | Secure Convex functions, authorization, internal operations, scheduled work, storage, and data access. | `secod-convex` | 1 |
| Dodo Payments | Secure Dodo Payments events, deduplication, subscriptions, entitlements, refunds, and disputes. | `secod-dodo-payments` | 1 |
| Firebase | Secure Firebase Rules, App Check, Admin SDK access, authentication, data, and supported services. | `secod-firebase` | 1 |
...
| Polar | Secure Polar checkout, customer binding, provider state, subscriptions, and entitlements. | `secod-polar` | 1 |
| Stripe | Secure Stripe keys, environment isolation, verified events, customers, subscriptions, and entitlements. | `secod-stripe` | 1 |
| Supabase | Secure Supabase authentication, Postgres roles and RLS, Storage, Realtime, service credentials, and tenant authorization. | `secod-supabase`<br>`secod-supabase-auth` | 2 |
| Vercel | Secure Vercel deployments and platform configuration plus AI SDK and AI Gateway provider, model, budget, and telemetry controls. | `secod-vercel-platform`<br>`secod-vercel-ai` | 2 |
| Whop | Secure Whop credentials, verified membership state, billing events, and access entitlements. | `secod-whop` | 1 |
