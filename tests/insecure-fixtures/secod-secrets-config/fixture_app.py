...
def scan_surfaces(files: Mapping[str, str]) -> list[dict[str, object]]:
    ...
    for path, content in files.items():
        if FAKE_KEY_PATTERN.search(content):
            findings.append(finding(1, "unsafe pattern reproduced", path, "credential-like literal"))
        if re.search(r"logger\.(?:info|debug|error)\([^)]*SECRET_VALUE", content):
            findings.append(finding(1, "unsafe pattern reproduced", path, "credential interpolated into log"))
    return findings
...
def check_bearer_flow(
    ...
    if {"session_token", "share_verifier"} & set(response_fields):
        findings.append(finding(2, "unsafe pattern reproduced", "fixture/response.json", "bearer in response"))
    if {"tenant_capability", "provider_token"} & set(rpc_arguments):
        findings.append(
            finding(2, "unsafe pattern reproduced", "fixture/rpc.schema", "client-supplied capability")
        )
    return findings
...
def check_scope(role: str, operations: Iterable[str]) -> list[dict[str, object]]:
    ...
    if role in {"superuser", "admin", "service_role"} and set(operations) <= routine_operations:
        return [finding(3, "secure implementation required", "fixture/runtime.config", "overprivileged role")]
    return []
...
def check_template(
    ...
    for name, value in template.items():
        if FAKE_KEY_PATTERN.search(value):
            findings.append(
                finding(4, "unsafe pattern reproduced", ".env.example", f"live-format value for {name}")
            )
...
def check_storage(
    ...
    if public_secret:
        findings.append(
            finding(5, "unsafe pattern reproduced", "fixture/client.env", "secret-class name exposed to client")
        )
    if compose_password is not None:
        findings.append(finding(5, "unsafe pattern reproduced", "fixture/compose.yaml", "literal password"))
    return findings
...
def check_environment_separation(
    ...
    if staging_host == "production-db.fixture.invalid":
        findings.append(
            finding(6, "unsafe pattern reproduced", "fixture/staging.env", "staging targets production host")
        )
    if {"test", "live"} <= set(key_modes):
        findings.append(finding(6, "unsafe pattern reproduced", "fixture/provider.env", "mixed test/live identities"))
    return findings
...
def check_rotation(
    ...
    if not has_revocation_path:
        return finding(7, "secure implementation required", "fixture/rotation-record.json", "no revocation path")
    ...
    return finding(7, "secure pattern confirmed", "fixture/rotation-record.json", "complete current record")
...
def check_production_flags(
    ...
    if bypass_default:
        findings.append(
            finding(8, "unsafe pattern reproduced", "fixture/auth.config", "bypass is fail-open when unset")
        )
    if debug_enabled:
        findings.append(finding(8, "secure implementation required", "fixture/production.env", "debug enabled in production"))
    return findings
...
def check_history_response(events: Iterable[str]) -> dict[str, object]:
    ...
    if "revoke" not in ordered:
        return finding(9, "unsafe pattern reproduced", "fixture/history-plan.json", "credential not revoked")
    ...
    return finding(9, "secure pattern confirmed", "fixture/history-plan.json", "revoke-first order")
...
def check_default_credentials(
    ...
    if seed_uses_default:
        return finding(10, "secure implementation required", "fixture/seed.sql", "known default credential")
    if not probe_authorized:
        ...
        return result
    return finding(10, "secure pattern confirmed", "fixture/deployment.json", "authorized evidence supplied")
